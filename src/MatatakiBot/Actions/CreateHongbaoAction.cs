...
格式：`[数额] [Fan 票符号]`", ParseMode.Markdown);
            ...
            var message = await _botClient.SendTextMessageAsync(callbackQuery.Message.Chat.Id, "准备红包中……");
            ...
            await _botClient.EditMessageTextAsync(callbackQuery.Message.Chat.Id, message.MessageId, "Ok");
        }
    }
}
